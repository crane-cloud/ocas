...
use crate::algorithms::OAlgorithmSerialisedExport;
use crate::core::{OIndividual, OIndividuals, OOError, OObjective, OObjectiveDirection, OProblem};
...
pub(crate) fn check_ref_point_coordinate(
    ...
    if (objective.direction() == OObjectiveDirection::OMinimise) & (ref_point_coordinate <= max_obj) {
        return Err(
            format!(
    ...
    } else if (objective.direction() == OObjectiveDirection::OMaximise)
...
impl HyperVolume {
    ...
    pub fn from_values(
        ...
        let mut new_individuals: Vec<OIndividual> = vec![];
    ...
    pub fn from_file<AlgorithmOptions: Serialize + DeserializeOwned>(
        data: &OAlgorithmSerialisedExport<AlgorithmOptions>,
        reference_point: &[f64],
    ) -> Result<HyperVolumeFileData, OOError> {
        let problem: OProblem = data.problem()?;
    ...
    pub fn from_files<AlgorithmOptions: Serialize + DeserializeOwned>(
        data: &[OAlgorithmSerialisedExport<AlgorithmOptions>],
        reference_point: &[f64],
    ) -> Result<AllHyperVolumeFileData, OOError> {
        let mut results = data
            .iter()
            .map(|p| HyperVolume::from_file::<AlgorithmOptions>(p, reference_point))
            .collect::<Result<Vec<HyperVolumeFileData>, OOError>>()?;
    ...
    pub fn estimate_reference_point_from_files<AlgorithmOptions: Serialize + DeserializeOwned>(
        ...
        let results = data
            ...
            .collect::<Result<Vec<Vec<f64>>, OOError>>()?;
