...
///
/// # Examples
pub fn vector_min(v: &[f64]) -> Result<f64, OOError> {
    Ok(*v
        .iter()
        .min_by(|a, b| a.total_cmp(b))
        .ok_or(OOError::Generic(
...
///
/// returns: `Result<f64, OError>`
pub fn vector_max(v: &[f64]) -> Result<f64, OOError> {
    Ok(*v
        .iter()
...
pub fn hasmap_eq_with_nans<K, S>(a: &HashMap<K, f64, S>, b: &HashMap<K, f64, S>) -> bool
where
    K: Eq + Hash,
    S: BuildHasher,
{
    ...
    a.iter().all(|(key, value)| {
        b.get(key)
            .map_or(false, |v| (v.is_nan() && value.is_nan()) || (*value == *v))
    })
}
